<div class="wizard mb-0" data-initialize="wizard" id="{{$_id}}">
    ...
    <ul class="steps">
      @foreach($_steps as $s)
        <li data-step="{{$loop->iteration}}" class = "{{ str_slug($s['name']) }}-step @if($loop->first) active @endif">
	...
	<div class="step-content pt-0 mb-1 pb-0" style = "@if(isset($_min_height)) min-height:{{ $_min_height }} @endif">
		@foreach($_steps as $s)
			<div class="{{ str_slug($s['name'], '-') }}-step-content mt-2 container-fluid mb-0 step-pane @if($loop->first) active @endif " data-step="{{ $loop->iteration}}">
