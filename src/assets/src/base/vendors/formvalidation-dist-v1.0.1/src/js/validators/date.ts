...
export default function date(): ValidateFunction {
    /**
    ...
    return {
        validate(input: ValidateInput<DateOptions, DateLocalization>): ValidateResult {
            ...
            const min = (opts.min instanceof Date)
                        ? (opts.min as Date)
                        : (opts.min ? parseDate(opts.min as string, dateFormat, separator) : d);
            const max = (opts.max instanceof Date)
                        ? (opts.max as Date)
                        : (opts.max ? parseDate(opts.max as string, dateFormat, separator) : d);
            // In order to avoid displaying a date string like "Mon Dec 08 2014 19:14:12 GMT+0000 (WET)"
            const minOption = (opts.min instanceof Date) ? formatDate(min, opts.format) : (opts.min as string);
            const maxOption = (opts.max instanceof Date) ? formatDate(max, opts.format) : (opts.max as string);
